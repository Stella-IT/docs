...
const { data: posts } = await useAsyncData('posts-list', () => queryContent('ko').find())
articles.value = posts.value.filter(v => v.category && v.category.toLowerCase() == props.pageSlug.toLowerCase())
